<#-- Standalone login.ftl (no imports). Safe guards + DaisyUI link. -->
...
<#assign themeName = "neutral">
<#if properties?? && properties.myTheme??>
  <#assign themeName = properties.myTheme>
</#if>

<!doctype html>
<html lang="${currentLang}" dir="${isRtl?string('rtl','ltr')}" data-theme="adconnect">
<head>
  <meta charset="utf-8"/>
  <meta name="viewport" content="width=device-width,initial-scale=1"/>
  <title>${msg("doLogIn")}</title>

<link rel="stylesheet" href="${url.resourcesPath}/css/daisy.min.css?v=9"/>
<link rel="stylesheet" href="${url.resourcesPath}/css/overrides.css?v=9"/>
  ...
  <div class="w-full max-w-md px-4">
    <div class="card bg-base-100 shadow-xl">
      <div class="card-body">
        ...
        <#-- Global message banner (errors / info) -->
        <#if message??>
          <div class="alert <#if message.type?? && message.type?lower_case == 'error'>alert-error<#elseif message.type?? && message.type?lower_case == 'warning'>alert-warning<#else>alert-info</#if> mt-2">
        ...
        <form id="kc-form-login" action="${url.loginAction}" method="post" class="mt-4">
          <div class="form-control">
            <label class="label" for="username"><span class="label-text">${msg("username")}</span></label>
          ...
          <div class="form-control mt-3">
            <label class="label" for="password"><span class="label-text">${msg("password")}</span></label>
  ...
  <script <#if cspNonce??>nonce="${cspNonce}"</#if>>
    function setLocale(loc){
      try{
        ...
        window.location.href = window.location.href + sep + 'kc_locale=' + encodeURIComponent(loc) + '&ui_locales=' + encodeURIComponent(loc);
      }
    }
  </script>
</body>
